...
and hover through `K` in normal mode for Rust files. Opened Rust buffers keep their document
state synchronized with
...
results open in a read-only popup near the cursor and dismiss on the next keypress.
