...
- Editing: insert text, generic `d`/`c`/`y`/`=` operator bindings such as `dw`, `cE`, `ye`, `==`, `=iw`, `dfx`, `ct,`, aliases such as `D`/`C`, line joining with `J`, single-character replace with `r`, number increment/decrement with `Ctrl+A` / `Ctrl+X`, characterwise, linewise, and blockwise visual selections, and mirrored visual `I` / `A` inserts across touched lines
